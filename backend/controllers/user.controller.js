...
export const getCurrentUser = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId)
      .select("-password")
      .populate("posts loops posts.author posts.comments followers following");
...
export const suggestedUser = async (req, res) => {
  try {
    ...
    return res.status(500).json(`seggestedUser error ${error}`);
  }
};
...
export const getAllNotification = async (req, res) => {
  try {
    ...
    console.log(error);
    return res.status(500).json({ message: "get all notification error" });
  }
};
...
export const markAsRead = async (req, res) => {
  try {
    ...
    console.log(error);
    return res.status(500).json({ message: "mark as read error" });
  }
};
